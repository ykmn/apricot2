...
channels:
  - id: retrofm_pgm
    ...
    playlogs:
      - retrofm
    playlogs_delay: 500        # задержка плейлога относительно аудио в мс (по умолчанию 500)
```

Путь на сервере формируется как: `\\host\share\path\<папка-даты>\<файл-время>.ext`.

**`playlogs_delay`** — компенсация задержки плейлога относительно аудиозаписи. Плейлог фиксирует момент постановки трека в эфир, тогда как в записи он слышится чуть позже (задержки кодирования, буферизации, сигнальной цепи). Значение в миллисекундах: метки плейлога сдвигаются вперёд на это количество секунд, выравниваясь с реальным положением трека в аудио. По умолчанию — 500 мс.
...
> [!NOTE]
> Если `admin_groups` и `access_groups` не заданы — любой успешно прошедший аутентификацию доменный пользователь получает доступ пользователя (только навигация и экспорт без прав администратора).
