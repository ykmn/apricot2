...
> [!WARNING]
> **Не используйте `Domain Users` в качестве группы доступа.** `Domain Users` — первичная группа пользователя в AD: она хранится в атрибуте `primaryGroupID`, а не в `memberOf`, и поэтому **не видна** при LDAP-запросе членства. Пользователь пройдёт аутентификацию, но получит отказ при проверке групп. Создайте отдельную security-группу (например `CN=AppUsers,OU=Groups,DC=corp,DC=local`), добавьте в неё нужных пользователей и укажите её в `access_groups`.
