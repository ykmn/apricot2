...
Точки автоматически пересобираются при «Обновить конфигурацию без перезапуска» — удалённый из конфига канал сразу исчезает из строки, добавленный сразу появляется, без обновления страницы. В одной строке — не более 20 точек, при большем числе каналов точки переносятся на следующую строку.
...
Источники проверяются по одному, как и аудио-каналы — точки заполняются постепенно, а не все разом. Проверка запускается автоматически после загрузки плейлогов при старте и по нажатию «Пересканировать плейлоги станции». В одной строке — не более 20 точек.
