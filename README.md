...
После успешного входа в браузер устанавливается cookie-сессия. Продолжительность сессии задаётся параметром `server.session_ttl` в `config/settings.yaml` (в секундах, по умолчанию **604800 = 7 дней**). Сессии хранятся в памяти сервера — при перезапуске сбрасываются.
