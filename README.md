...
- **Скачать всё** — открывает диалог с выбором:
  - **Скачать отдельными файлами** — экспортирует и скачивает все фрагменты из списка последовательно отдельными файлами.
  - **Склеить фрагменты в один файл** — объединяет все выбранные фрагменты в один аудиофайл; порядок склейки на выбор — либо по времени начала фрагмента, либо по очереди в списке. Внутри файла расставляются маркеры с датой-временем начала каждого исходного фрагмента:
    - MP3 — главы (ID3v2 CHAP/CTOC, читаются большинством плееров) **и** маркеры в собственном формате Sound Forge (`GEOB:SfMarkers`), которые не показывают обычные плееры, но открываются в Sound Forge/аналогичных редакторах — записываются оба варианта одновременно
...
Имя склеенного файла: `{Название канала|Несколько каналов} склейка с {YYYY-mm-dd} {HH-MM-SS}.{fmt}`
...
- **⏸** — пауза
- **×1.5** — переключатель ускоренного воспроизведения (в 1,5 раза быстрее); режим остаётся включённым между запусками плеера, пока не выключен повторным нажатием
...
- **Восстановление состояния** — при повторном открытии страницы автоматически восстанавливаются выбранный канал, позиция таймлайна, маркеры выделения и список фрагментов. Состояние сохраняется на сервере в `config/sessions-ui.yaml` и привязано к имени пользователя, поэтому работает в любом браузере и на любом устройстве.
...
| 🔍 Пересканировать аудио текущего канала | Принудительно перечитать список аудиофайлов | Администраторы |
| 📋 Пересканировать плейлоги | Очистить кэш плейлогов за последние 2 дня и перепроверить все источники плейлогов текущей станции | Администраторы |
| 🔄 Обновить конфигурацию без перезапуска | Перечитать YAML-файлы без перезапуска процесса на сервере | Администраторы |
| 👥 Сессии | Отобразить окно текущих активных сессий пользователей с возможностью принудительно завершить сессию | Администраторы |
| 🔎 Проверить обновления | Проверить наличие новых коммитов на `update:repo_url` из `settings.yaml`, обновить и автоматически перезапустить приложение | Администраторы |
...
> [!INFO]
> Приложение разработано заново по отдельному техническому заданию и, кроме элементов технического задания, не имеет общего кода с проектом _"Абрикос"_ разработки Микро-ИТ.
