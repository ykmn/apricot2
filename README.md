...
Точки автоматически пересобираются при «Обновить конфигурацию без перезапуска» — удалённый из конфига канал сразу исчезает из строки, добавленный сразу появляется, без обновления страницы. В одной строке — не более 50 точек (с увеличенным промежутком через каждые 10 для удобства подсчёта); при большем числе каналов лишние точки образуют отдельную строку снизу, заполняясь справа налево — крайняя правая группа из 10 всегда полная, неполная группа оказывается слева.
